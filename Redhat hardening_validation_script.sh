...
r_awx_shadow_hyphen_perm_all="0  root  root"
...
if [[ $c_awx_shadow_hyphen_perm == "0" ]] && [[ $c_awx_shadow_hyphen_user == "root" ]] && [[ $c_awx_shadow_hyphen_group == "root" ]]; then
...
r_awx_gshadow_hyphen_perm_all="0  root  root"
...
if [[ $c_awx_gshadow_hyphen_perm == "0" ]] && [[ $c_awx_gshadow_hyphen_user == "root" ]] && [[ $c_awx_gshadow_hyphen_group == "root" ]]; then
...
## Sticky bit not set for /tmp and other world writable directories. ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
## Validate NTP/Chriny Configurations
...
elif [ -f  /etc/chrony.conf ] && [ `systemctl is-active chronyd` == "active" ]; then
...
chrony_count=$(cat /etc/chrony.conf | egrep -i "^server|^pool" | wc -l)
  if [ $chrony_count -gt 0 ]; then
     c_ntp_config=`echo "NTP Configuration is being done in /etc/chrony.conf<br/><br/>"; cat /etc/chrony.conf | egrep -i "^server|^pool"`
     s_ntp_config="Compliant"
  else
     c_ntp_config="NTP Configuration is Not done in /etc/chrony.conf"
     s_ntp_config="Non-Compliant"
  fi
else
  c_ntp_config="NTP Configuration is Not done in /etc/chrony.conf"
...
r_awx_packages_all="Remove/Disable below Services<br/><br/>telnet<br/>telnet-server<br/>vsftpd<br/>tftp<br/>rsh_server<br/>rsh"
r_awx_nfs_server_status="NFS-Server service should be in in-active state"
r_awx_firewalld_status="Firewalld Service status should be in in-active state"
r_sestatus="Selinux Status should be in disabled state"


c_awx_telnet=$(rpm -qa --qf "%{NAME}\n" telnet)
c_awx_telnet_server=$(rpm -qa --qf "%{NAME}\n" telnet-server)
c_awx_vsftpd=$(rpm -qa --qf "%{NAME}\n" vsftpd)
c_awx_tftp=$(rpm -qa --qf "%{NAME}\n" tftp)
c_awx_nfs_server=$(systemctl is-active nfs-server)
c_awx_firewalld=$(systemctl is-active firewalld)
c_awx_rsh_server=$(rpm -qa --qf "%{NAME}\n" rsh-server)
c_awx_rsh=$(rpm -qa --qf "%{NAME}\n" rsh)
c_sestatus=$(sestatus | grep "^SELinux status:" | awk -F: '{print $2}' | awk '{$1=$1};1')


if [ `rpm -qa --qf "%{NAME}\n" telnet` ] || [ `rpm -qa --qf "%{NAME}\n" telnet-server` ] || [ `rpm -qa --qf "%{NAME}\n" vsftpd` ] || [ `rpm -qa --qf "%{NAME}\n" tftp` ] || [ `rpm -qa --qf "%{NAME}\n" rsh-server` ] || [ `rpm -qa --qf "%{NAME}\n" rsh` ]; then
   c_awx_packages_all=`echo "Some packages are Installed on server<br/><br/>$c_awx_telnet<br/>$c_awx_telnet_server<br/>$c_awx_vsftpd<br/>$c_awx_tftp<br/>$c_awx_rsh_server<br/>$c_awx_rsh"`
...
if [ $c_awx_firewalld == "active" ]; then
   c_awx_firewalld_status="Firewalld service is -- $c_awx_firewalld"
   s_awx_firewalld_status="Non-Compliant"
else
   c_awx_firewalld_status="Firewalld service is -- $c_awx_firewalld"
   s_awx_firewalld_status="Compliant"
fi
...
c_awx_pwd_complex_difok=$(cat /etc/pam.d/password-auth | egrep -Eo "difok=2")

r_awx_pwd_complex_dcredit="dcredit=-1"
c_awx_pwd_complex_dcredit=$(cat /etc/pam.d/password-auth | egrep -Eo "dcredit=-1")

r_awx_pwd_complex_ucredit="ucredit=-1"
c_awx_pwd_complex_ucredit=$(cat /etc/pam.d/password-auth | egrep -Eo "ucredit=-1")

r_awx_pwd_complex_ocredit="ocredit=-1"
c_awx_pwd_complex_ocredit=$(cat /etc/pam.d/password-auth | egrep -Eo "ocredit=-1")

r_awx_pwd_complex_lcredit="lcredit=-1"
c_awx_pwd_complex_lcredit=$(cat /etc/pam.d/password-auth | egrep -Eo "lcredit=-1")

r_awx_account_lockout="deny=5"
c_awx_account_lockout=$(cat /etc/pam.d/password-auth | egrep -Eo "deny=5" | wc -l)
c_awx_account_lockout_all=$(cat /etc/pam.d/password-auth | egrep -Eo "deny=5" | head -n1)
...
r_awx_syslog_messages="/var/log/messages"
r_awx_syslog_secure="/var/log/secure"
r_awx_syslog_cron="/var/log/cron"
r_awx_syslog_boot="/var/log/boot.log"
r_awx_syslog_cmdlog="/var/log/cmdlog"

if [ -f /etc/rsyslog.conf ];then


c_awx_syslog_messages=$(cat /etc/rsyslog.conf | grep "/var/log/messages" | wc -l)
c_awx_syslog_secure=$(cat /etc/rsyslog.conf | grep "/var/log/secure" | wc -l)
c_awx_syslog_cron=$(cat /etc/rsyslog.conf | grep "/var/log/cron" | wc -l)
c_awx_syslog_boot=$(cat /etc/rsyslog.conf | grep "/var/log/boot.log" | wc -l)
c_awx_syslog_cmdlog=$(cat /etc/rsyslog.conf | grep "/var/log/cmdlog" | wc -l)


r_awx_syslog_all="*.info;mail.none;authpriv.none;cron.none                /var/log/messages
authpriv.*                                              /var/log/secure
cron.*                                                  /var/log/cron
local7.*                                                /var/log/boot.log
local6.notice                                           /var/log/cmdlog"

        if [ $c_awx_syslog_messages -gt 0 ] && [ $c_awx_syslog_secure -gt 0 ] && [ $c_awx_syslog_cron -gt 0 ] && [ $c_awx_syslog_boot -gt 0 ] && [ $c_awx_syslog_cmdlog -gt 0 ]; then
